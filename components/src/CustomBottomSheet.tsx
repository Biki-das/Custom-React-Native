...
const data = [...Array(10).keys()].map(() => {
  ...
  return {
    ...
    tags: [...Array(faker.datatype.number(3) + 2).keys()].map(() =>
      ...
      transform: [
        {
          translateY: interpolate(
            animatedIndex.value,
            [0, 1],
            [-itemY.value + (_spacing * 5 + 48) * index, 0],
          ),
        },
      ],
    };
  ...
  const headingStyles = useAnimatedStyle(() => {
    return {
      left: interpolate(
        animatedIndex.value,
        [0, 1],
        [_spacing * 2, width / 2 - headingWidth.value / 2],
        Extrapolate.CLAMP,
      ),
      top: interpolate(
        animatedIndex.value,
        [0, 1],
        [0, (headerHeight + Constants.statusBarHeight - 24) / 2],
        Extrapolate.CLAMP,
      ),
    };
  });
      ...
      transform: [
        {
          translateY: interpolate(
            animatedIndex.value,
            [0, 1],
            [0, headerHeight + Constants.statusBarHeight],
            Extrapolate.CLAMP,
          ),
        },
      ],
    ...
    <View style={styles.container}>
      ...
      <Animated.View
        ...
        <View style={{paddingHorizontal: _spacing * 2}}>
          <Animated.View
            style={[
              {position: 'absolute', left: 0, flexDirection: 'row'},
              headingStyles,
            ]}
            onLayout={onLayout}>
            <Text style={{fontWeight: '800', color: 'white', fontSize: 28}}>
              Discover
            </Text>
            <Text style={{fontWeight: '800', color: '#F60822', fontSize: 28}}>
              .
            </Text>
          </Animated.View>
          <Animated.Text
            style={[{color: 'white', marginTop: 50}, subHeadingStyles]}>
            Discover design article,{'\n'}illustrations & interfaces.
          </Animated.Text>
        </View>
      </Animated.View>
        ...
        <BottomSheet
          ...
          <BottomSheetFlatList
            ...
            renderItem={({item, index}) => {
              return (
                <Item item={item} animatedIndex={animatedIndex} index={index} />
              );
            }}
          />
        </BottomSheet>
